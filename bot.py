...
def gen_inline_markup(rows):
    ...
    for row in rows:
        row = str(row)  # ✅ защита
        markup.add(InlineKeyboardButton(text=row, callback_data=row))
    return markup
...
ℹ️ Ты также можешь *просто написать имя проекта*  
и я покажу всю информацию о нём 👇
        """,
        parse_mode="Markdown"
    )
...
def get_projects(message):
    ...
    if projects:
        ...
        bot.send_message(
            message.chat.id,
            text,
            reply_markup=gen_inline_markup([x[1] for x in projects])
        )
    else:
        no_projects(message)
