...
class DB_Manager:
    ...
    def get_projects(self, user_id):
        sql = '''
        SELECT project_id, project_name, description, url, status_name
        FROM projects
...
"""
    ...
    def update_projects(self, param, data):
        if param not in ('project_name', 'description', 'url', 'status_id'):
            raise ValueError("param должен быть одним из: project_name, description, url, status_id")
...
if __name__ == '__main__':
    ...
    status_id = manager.get_status_id('На этапе проектирования')
    manager.insert_project([
        (user_id, 'Telegram Bot', 'Бот для заметок', 'https://github.com/user/bot', status_id)
    ])

    manager.insert_skill(user_id, 'Telegram Bot', 'Python')
    manager.insert_skill(user_id, 'Telegram Bot', 'Telegram')
    manager.insert_skill(user_id, 'Telegram Bot', 'SQL')

    print("Проекты пользователя:", manager.get_projects(user_id))

    print("Инфо по проекту:", manager.get_project_info(user_id, 'Telegram Bot'))
    print("Навыки проекта:", manager.get_project_skills('Telegram Bot'))

    project_id = manager.get_project_id('Telegram Bot', user_id)
